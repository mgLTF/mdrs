...
error_interference = ( p_nerrs(4, 1:length(x)).*pstates(4) + p_nerrs(5, 1:length(x)).*pstates(5)) ./ (p_nerrs(1, 1:length(x)) .* pstates(1) + p_nerrs(2, 1:length(x)) .* pstates(2) + p_nerrs(3, 1:length(x)) .* pstates(3) + p_nerrs(4, 1:length(x)) .* pstates(4) + p_nerrs(5, 1:length(x)) .* pstates(5));

figure(3);
semilogy(x, error_interference, 'b');
xlabel('B (Bytes)');
...
rate = 1000;            %pps
capacity = 10 * 10^6;   % bits p/ seg
delay = 10*10^-6;
x = 64:1518;

%% 4.a)
prob_rem = (1 - 0.19 - 0.23 - 0.17) / (length(x) - 3);
avg_size = 64*0.19 + 110*0.23 + 1518*0.17 + sum( (65:109) * prob_rem ) + sum( (111:1517) * prob_rem);

% 620 bytes = 620 * 8 bits

% 10 Mbps 

avg_packet_transmission_time = avg_size * 8 / capacity

%% 4. b)

% find throughput
% 1000 packets per second
% each packet has avg size of ~ 620 bytes
% throughput = 1000 * (620 * 8) bits per sec

throughput = rate * avg_size * 8 / 1e6 % Mbps


%% 4. c)

% capacity = 10 * 10^6 bps
% avg_packet_size ~= 620 bytes

capacity_pps = capacity / (avg_size*8) % pps
